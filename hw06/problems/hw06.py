    ...
    if is_weight(m):
        ...
        for i in sides(m):
            if not balanced(i):
                return False
        w1,w2=0,0
        for i in branches(sides(m)[0]):
            w1=w1+total_weight(i)
        for j in branches(sides(m)[1]):
            w2=w2+total_weight(j)
        if w1*length(sides(m)[0])==w2*length(sides(m)[1]):
            return True
        else:
            return False
    else:
...
def make_withdraw(balance, password):
    ...
    "*** YOUR CODE HERE ***"
    ans,count=[],0
    def withdraw(amount, password_this):
        nonlocal balance,password,ans,count
        if count>=3:
            return 'Your account is locked. Attempts: '+str(ans)
        else:
            if password==password_this:
                if amount>balance:
