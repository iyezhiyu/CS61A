...
def make_pytunes(username):
    ...
    """
    "*** YOUR CODE HERE ***"
    ans = tree(username,[tree('pop',[tree('justin bieber',[tree('single',[tree('what do you mean?')])]),tree('2015 pop mashup')]),tree('trance',[tree('darude',[tree('sandstorm')])])])
    return ans
...
def add_song(t, song, category):
    ...
    "*** YOUR CODE HERE ***"
    if root(t) == category:
        return [root(t)]+[i for i in branches(t)]+[[song]]
    elif not is_leaf(t):
        ans=[root(t)]
        for branch in branches(t):
            ans += [add_song(branch,song,category)]
        return ans
    else:
        return t
