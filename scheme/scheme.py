...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        args=expr.second.map(lambda x: scheme_eval(x, env))
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        env.bindings[target.first]=do_lambda_form(expressions, env)
        return target.first
        # END PROBLEM 10
    else:
...
def make_let_frame(bindings, env):
    ...
    while bindings != nil:
        tem=bindings.first
        check_form(tem, 2, 2)
        check_formals(Pair(tem.first, nil))
        child.define(tem.first, scheme_eval(tem.second.first, env))
