def deep_len(lnk):
    ...
    if lnk.rest == lnk.empty:
        if isinstance(lnk.first,int):
            ...
            ans+=deep_len(lnk.first)
    else:
        if isinstance(lnk.first,int):
            ans+=1
        elif isinstance(lnk.first,Link):
            ans+=deep_len(lnk.first)
        ans+=deep_len(lnk.rest)
    return ans
...
def make_to_string(front, mid, back, empty_repr):
    ...
    def interesting(lnk):
        if lnk is Link.empty:
            ...
            return front+str(lnk.first)+mid+interesting(lnk.rest)+back
    return interesting
...
def tree_map(fn, t):
    ...
    if t.is_leaf():
        ...
        for i in range(len(t.branches)):
             t.branches[i]=tree_map(fn, t.branches[i])
        return t
...
def add_trees(t1, t2):
    ...
    if (not t1.is_leaf()) and (not t2.is_leaf()):
        newtree.root=t1.root+t2.root
        min_len=min(len(t1.branches),len(t2.branches))
        for i in range(min_len):
            newtree.branches.append(add_trees(t1.branches[i],t2.branches[i]))
        if len(t1.branches)>len(t2.branches):
            for i in t1.branches[len(t2.branches):len(t1.branches)]:
                newtree.branches.append(i)
        elif  len(t1.branches)<len(t2.branches):
            for i in t2.branches[len(t1.branches):len(t2.branches)]:
                newtree.branches.append(i)
        return newtree
    elif t1.is_leaf() and t2.is_leaf():
        newtree.root=t1.root+t2.root
        return newtree
    elif t1.is_leaf() and (not t2.is_leaf()):
        newtree.root=t1.root+t2.root
        for i in t2.branches:
            newtree.branches.append(i)
        return newtree
    elif (not t1.is_leaf()) and t2.is_leaf():
        newtree.root=t1.root+t2.root
        for i in t1.branches:
            newtree.branches.append(i)
        return newtree
